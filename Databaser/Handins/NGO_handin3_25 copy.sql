...
# a)
SELECT S.name, sum(D.amount) AS total
FROM Supporter AS S, Donations AS D
WHERE S.id = D.s_id AND S.NGO_name = 'KDG'
GROUP BY S.name
...
WHERE S.id = D.s_id 
GROUP BY S.name
HAVING count(D.s_id) = 1 + (SELECT ROUND(avg(don_count)) 
							FROM (SELECT count(*) AS don_count
									FROM Donations
									WHERE NGO_name = 'KDG'
									GROUP BY s_id) AS average);

# c)
SELECT S.name, D.ngo_name
FROM Supporter AS S, Donations as D
WHERE S.id = D.s_id 
GROUP BY D.ngo_name, S.name
HAVING count(D.s_id) = 1 + (SELECT ROUND(avg(don_count)) 
							FROM (SELECT count(*) AS don_count
									FROM Donations AS D2
									WHERE D.ngo_name = D2.ngo_name
									GROUP BY s_id) AS average)
ORDER BY D.ngo_name;
