...
`email` 		VARCHAR(30) 			NOT NULL 	CHECK (email LIKE '%_@_%.__%')		UNIQUE,
...
# 2a)
INSERT INTO Supporter VALUES (12345, 'Jens Jensen', 'jensen@yahoo.dk', '12345678', 'Tilst', 'MDGA', '1968-11-29', TRUE);
...
# 3a)
SELECT N.cause, N.director
FROM NGO AS N, supporter AS S
WHERE N.name = S.NGO_name AND S.name = 'Ira Assent';

# 3b)
SELECT name, phone
FROM supporter
WHERE city LIKE '%Aarhus%';

# 3c)
SELECT S.name, S.email
FROM supporter AS S, Donations AS D
WHERE S.id = D.s_id AND D.amount >= 968
ORDER BY S.name DESC;

# 3d)
SELECT DISTINCT S.name, S.phone
FROM supporter AS S, Donations AS D
WHERE S.id = D.s_id AND D.activity = 'Water is a human right';
